package com.qkbus.modules.monitor.service.impl;
...
import com.qkbus.modules.monitor.service.mapper.VisitsMapper;
...
public class VisitsServiceImpl extends BaseServiceImpl<VisitsMapper, Visits> implements VisitsService {
    ...
    public Object get() {
        ...
        Visits visits = this.getOne(new LambdaQueryWrapper<Visits>()
                .eq(Visits::getDate, localDate.toString()));
        List<Visits> list = this.query().between("gmt_create", localDate.minusDays(6).toString(), localDate.plusDays(1).toString()).list();
